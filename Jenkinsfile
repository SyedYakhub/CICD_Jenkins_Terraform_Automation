    ...
    stages {
        stage ('ssh connect + git checkout + terraform apply') {
            steps {
                ...
                sh 'ssh -o StrictHostKeyChecking=no ec2-user@172.31.15.49 cd /var/lin/jenkins/CICD_Jenkins_Terraform_Automation && terraform init && terraform plan && terraform apply --auto-approve && terraform destroy --auto-approve'
